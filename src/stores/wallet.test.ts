...
import { ACCOUNTS_PATH } from '@/utils/constants'
...
describe('wallet store, create new wallet', () => {
  ...
  it('setMiner', () => {
    expect(() => {
      wallet.setMiner('0x00001B0e12d3b39F606f2B83d5c78d2C3A41A9CC91CF')
    }).not.toThrow()
  })

  it('getCurrentBlock', async () => {
    await wallet.getCurrentBlock()
    expect(wallet.blockInfo).toEqual({
      ...blockInfo.header,
      transactions: blockInfo.body.transactions.length
    })
  })
